...
def Torsion():
    ...
    if Datos.torsion_variable == True:
        ...
        while r <= (Datos.D)/2:
            if r in r_datos.values:
                # Obtener el índice del valor de r en la columna
                idx = r_datos[r_datos == r].index[0]
                ...
                r_upper = r_datos[r_datos >= r].min()
                # Encontrar el valor inmediatamente inferior en la lista
                r_lower = r_datos[r_datos <= r].max()
                ...
                valor = Beta_lower + ((Beta_upper-Beta_lower)/(r_upper-r_lower))*(r-r_lower)
...
#CUERDA
#Esta función lee en Datos si la torsión es variable o fija. Si es variable coge los datos de cuerda para cada radio
    ...
    if Datos.cuerda_variable == True:
        ...
        while r <= (Datos.D)/2:
            if r in r_datos.values:
                ...
                valor = cuerda_lower + ((cuerda_upper-cuerda_lower)/(r_upper-r_lower))*(r-r_lower)
