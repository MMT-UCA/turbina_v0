...
""" factor_corrector_sub, factor_corrector_trans, factor_corrector_sup = Factor_corrector_Mach(1.2)

print(factor_corrector_trans)
...
for Mach in np.arange(0, 2 + 0.05, 0.05):

    if Mach in [0, 0.8, 1.05, 1.25, 2]:
        factores_correctores = {
            0: 1,
            0.8: 1.7885,
            1.05: 2.515,
            1.25: 1.6468,
            2: 1.387}
        factor = factores_correctores[Mach]
    elif Mach < 0.8:
        factor, factor_2, factor_3 = Factor_corrector_Mach(Mach)
    elif 0.85<= Mach <=1.21:
        factor_1, factor, factor_3 = Factor_corrector_Mach(Mach)
    else:
        factor_1, factor_2, factor = Factor_corrector_Mach(Mach)
